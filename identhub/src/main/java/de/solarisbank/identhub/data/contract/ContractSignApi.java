...
public interface ContractSignApi {
    ...
    Single<IdentificationDto> postAuthorize(
            @Path("identification_uid") final String identificationId
    );

    @PATCH("sign_documents/{identification_uid}/confirm")
    Single<IdentificationDto> postConfirm(
            @Path("identification_uid") final String identificationId,
            @Body final TransactionAuthenticationNumber tan
    );

    @GET("sign_documents/{document_uid}/download")
    Single<Response<ResponseBody>> getDocumentFile(
            @Path("document_uid") final String documentId
    );
}
